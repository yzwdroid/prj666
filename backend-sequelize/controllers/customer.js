...
module.exports = {
  ...
  login(req, res){
    ...
    return Customer.findOne({ where: { email: req.body.email } })
      .then((customer) => {
        ...
        if (!customer) {
          res.status(201).send({ message: "No customer found, please input correct email address." });
        }
      ...
      .catch((error) => res.status(400).json({ message: "Crash for finding a customer." }));
  },
  async resetPassword(req, res){
    ...
    return sendToken.freshToken(resettoken)
              .then(() => {
                res.status(200).json({ message: "Send reset Password email successfully." });
                var mailOptions = {
                  to: user.email,
  ...
  async newPassword(req, res) {
    ...
    return bcrypt.hash(req.body.newPassword, 10, (err, hash) => {
      ...
      .catch((error) => {return res.status(400).json({ message: "Update password crash!" })});
